...
function TableBlogCategory({ data, triggerFetching }) {
  ...
  const handleEditConfirmation = async () => {
    try {
        ...
        if (!editedItem.name_blog_category || !editedItem.status || !editedItem.thumbnail) {
          ToastMessage('Vui lòng nhập đầy đủ thông tin').warn();
          return; // Ngừng xử lý nếu có bất kỳ trường nào chưa được nhập
        }
      const formData = new FormData();
      formData.append('name_blog_category', editedItem.name_blog_category);
      ...
      const response = await ServerApi.patch(`/blogCategory/${editedItem.blog_category_id}`, formData);
  ...
  const deleteBlogCategory = async (blogCategoryId) => {
    try {
      await ServerApi.delete(`/blogCategory/${blogCategoryId}`);
      triggerFetching();
    } catch (error) {
    ...
    () => [
      {
        ...
        render: (item) => (
          <div className="flex items-center gap-2">
            ...
            <img src={`${serverEndpoint}blogCategory/thumbnail/${item.thumbnail}`} alt="" />
            </div>
            <div className="">
        ...
        render: (item) => (
          <div className="flex items-center gap-2">
            ...
            onClick={() => {
              ...
              const shouldDelete = window.confirm(`Bạn có chắc muốn xóa danh mục bài viết: ${item.name_blog_category}?` );
              if (shouldDelete) {
                // Gọi hàm xóa
                deleteBlogCategory(item.blog_category_id);
              }
            }}
          >
      ...
      {editWindowOpen && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-[2]">
          <div className="bg-white p-[20px] rounded-lg w-[600px] m-[20px]">
            ...
            <h2 className="text-center text-[20px] text-blue-500 font-[600]">Sửa danh mục bài viết: {editedItem.name_blog_category}</h2>
            <div className="mt-[10px] border-b-[3px] py-[10px] max-h-[500px] overflow-auto">
              <p className="mb-[10px]">
            <label>Tên:</label><br/>
               <input className="w-[100%] border-solblog_category_id border-2 border-indigo-600 p-[10px] rounded-[5px] my-[5px]" placeholder="Nhập tên danh mục bài viết" type="text" value={editedItem.name_blog_category} onChange={(e) => setEditedItem({ ...editedItem, name_blog_category: e.target.value })} />
               </p>
  ...
  <div>
   <img
  src={editedItem.imageUrl || `${serverEndpoint}blogCategory/thumbnail/${editedItem.thumbnail}`}
  alt="Preview"
